...
function DropdownNavigation ({ location, children, title, icon }) {
  ...
  React.useEffect(() => {
    if (openedTitle) { dispatch(toggleNavItem(false)); }
  ...
  return (
    <Navbar className="p-0">
      <div className={`${openedTitle === title ? styles.svgWhiteMobile : ''} 
        ${isActive ? 'selected-nav-point' : ''} menu-points`}
      onClick={() => dispatch(toggleNavItem(title))} >
        <div>
          {icon}
        ...
        <FontAwesomeIcon className="d-none d-xl-block" icon={openedTitle === title ? faAngleUp : faAngleDown}/>
      </div>
      <div className={`${openedTitle === title ? styles.activeNavbar : ''} 
        ${styles.transitionWidth} d-flex d-xl-none h-100 align-items-center position-absolute`}>
      ...
      <Collapse isOpen={openedTitle === title}>
        {React.Children.map(children, element => {
          if (!element) return null;
          return (
            <NavItem className={`${location.pathname === element.props.to ? styles.selectedSubPoint : ''}
              ml-3 d-none d-xl-block`} key={element.props.to}>
              {element}
            </NavItem>
          );
        })}
